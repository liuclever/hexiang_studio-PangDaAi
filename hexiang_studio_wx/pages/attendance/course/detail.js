// pages/attendance/course/detail.js
...
Page({
   ...
   */
  onLoad(options) {
    console.log('[课程详情] 页面加载, 参数:', options);
    console.log('[课程详情] 接收到的planId:', options.planId, '类型:', typeof options.planId);
    ...
    console.log('[课程详情] 设置到data中的planId:', this.data.planId);
  ...
  async initPageData() {
    try {
      ...
      console.error('[课程详情] 初始化失败:', error);
      wx.showToast({
        title: '加载失败',
        icon: 'error'
      });
  ...
  async loadData(isRefresh = false) {
    ...
    try {
      ...
      console.log('[课程详情] 发起API请求，planId:', planId, '页码:', page, '类型: course');
      
      const response = await http.get(`/wx/attendance/plan/${planId}/records`, {
        page,
        size: pagination.size,
        type: 'course'  // 明确指定为课程考勤
      });
      
      console.log('[课程详情] API响应:', response);
      ...
      if (response.code === 200 && response.data) {
        ...
        const newPagination = {
          ...
          hasMore: false // 由于后端返回的是完整数组，没有分页
        };
      ...
      console.error('[课程详情] 加载数据失败:', error);
      wx.showToast({
  ...
  async loadMoreData() {
    ...
    try {
      ...
      console.error('[课程详情] 加载更多失败:', error);
      // 回滚页码
      this.setData({
  ...
  async editBasicInfo() {
    ...
    // 找到当前课程在课程列表中的索引
    const currentCourseId = this.data.planDetail.courseId;
    const courseIndex = this.data.allCourses.findIndex(course => course.courseId === currentCourseId);
    ...
    this.setData({
      ...
      'editForm.courseId': currentCourseId || '',
      'editForm.courseIndex': courseIndex >= 0 ? courseIndex : 0,
      'editForm.courseDisplay': this.data.allCourses[courseIndex >= 0 ? courseIndex : 0]?.name || '',
  ...
  onCourseChange(e) {
    ...
    if (selectedCourse) {
      this.setData({
        'editForm.courseIndex': index,
        'editForm.courseId': selectedCourse.courseId,
        'editForm.courseDisplay': selectedCourse.name
      });
    }
  },
  ...
  async saveChanges() {
    ...
    try {
      ...
      const updateData = {
        ...
        type: 'course', // 明确指定为课程考勤
        courseId: parseInt(editForm.courseId),
  ...
  deleteCourse() {
    wx.showModal({
      title: '确认删除',
      content: '删除后无法恢复，确定要删除这个课程考勤吗？',
      confirmText: '删除',
      confirmColor: '#ff4757',
      success: (res) => {
        if (res.confirm) {
          this.performDeleteCourse();
        }
      }
    });
  },
