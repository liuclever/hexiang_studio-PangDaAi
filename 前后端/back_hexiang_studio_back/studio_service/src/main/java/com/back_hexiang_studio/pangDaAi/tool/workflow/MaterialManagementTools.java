...
public class MaterialManagementTools {
    ...
    public String requestCategoryDeletion(
        ...
        try {
            ...
            warning.append("⚠️【严重警告 - 分类删除确认】⚠️\n");
            warning.append("══════════════════════════════════════\n");
            warning.append("分类信息：\n");
            ...
            log.warn("⚠️ 用户 {} 请求删除分类 '{}' (ID: {}), 影响资料: {} 个", 
    ...
    public String requestMaterialDeletion(
        ...
        try {
            ...
            if (materialResults.size() > 1) {
                ...
                multipleFiles.append("⚠️ 发现 ").append(materialResults.size()).append(" 个同名资料：\n");
            ...
            // 构建详细的风险分析报告
            StringBuilder warning = new StringBuilder();
            warning.append("⚠️【严重警告 - 资料删除确认】⚠️\n");
            warning.append("══════════════════════════════════════\n");
            warning.append("资料信息：\n");
    ...
    public String confirmMaterialDeletion(
        ...
        try {
            ...
            for (Map<String, Object> material : materials) {
                ...
                log.debug("🗂️ 处理资料 ID: {}, 路径: {}, 大小: {} bytes", materialId, filePath, fileSize);
                
                // 第1步：删除物理文件
                if (StringUtils.hasText(filePath)) {
                    // 注意：这里应该调用文件服务删除实际文件
                    // fileService.deleteFile(filePath); // 实际项目中需要实现文件删除逻辑
                    log.debug("🗂️ 准备删除物理文件: {}", filePath);
                    deletedFiles++; // 假设删除成功
                }
