<template>
  <div class="activity-attendance-container">
    <!-- 搜索区域 -->
    ...
    <el-card shadow="hover" class="attendance-list-card">
      ...
      >
        ...
        <el-table-column prop="name" label="考勤名称" min-width="180" />
        ...
        <el-table-column prop="location" label="考勤地点" min-width="150" />
        ...
        <el-table-column label="签到情况" min-width="280">
          <template #default="scope">
...
const fetchAttendancePlans = async () => {
  ...
  try {
    ...
    console.log('活动考勤API原始响应:', res); // 调试日志
    ...
    if (res && res.code === 200) {
      ...
      ElMessage.error(res?.msg || '获取活动考勤计划失败');
      attendancePlans.value = [];
      total.value = 0;
    }
  } catch (error) {
    console.error('获取活动考勤计划失败:', error);
    ElMessage.error('获取活动考勤计划失败');
    attendancePlans.value = [];
    total.value = 0;
  } finally {
    loading.value = false;
